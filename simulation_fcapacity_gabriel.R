...
plot(seasonaldecomp) #decomposiÁ„o da sÈrie
...
tbats_agg <- (tbats_components[,2] + tbats_components[,3] + 
                tbats_components[,4]) #SÈrie com as componentes STL sem os ruÌdos
plot(tbats_agg[1:96],ylim = c(0,1),type = "l")
mc_tbats_series <-  tbats_agg + rnorm(n = 8760, mean = 0, sd = sd_tbats) #SÈrie gerada via simulaÁ„o de MonteCarlo dos resÌduos
lines(mc_tbats_series[1:24], ylim = c(0,1), type = "l",col="red")
lines(tbats_agg[1:24],ylim = c(0,1),type = "l",lw=2)


# Box.test(tbats_remainder, type = "Ljung-Box") #se p-valor <0.05, rejeito H0 (erros iid) com 5% de signific‚ncia
# plot(tbats_components[1:96,3],type = "l") # verificando sazonalidade
# 
# 
# plot(seasonaldecomp$errors) #resÌduos do modelo ajustado TBATS
...
plot(seasonaldecomp2) #decomposiÁ„o da sÈrie
